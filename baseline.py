...
def submission(y_pred: np.ndarray) -> str:
    ...
    Parameters:
    - y_pred: np.ndarray - vector with predicted values
...
def feature_importances_plot(
    X_train: df, model: Union[XGBRegressor, LGBMRegressor]
) -> Figure:
    """
...
def XGBRegressor_hyperparameter_tuning(X: df, y: np.ndarray) -> dict:
    """
    Perform hyperparameter tuning using Hyperopt for XGBoost or LightGBM
